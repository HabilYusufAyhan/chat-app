...
const postprofilepage = async function (req, res, next) {
  ...
  try {
    if (req.body.links2) {
      ...
      fs.writeFile(
        "./src/uploads/avatars/" + req.user.email + ".jpg",
        base64Data,
        "base64",
        (err) => {
          if (err) throw err;
    ...
    if (req.body.links1) {
      ...
      // Dosyayı sunucuya kaydet
      fs.writeFile(
        "./src/uploads/avatars/" + req.user.email + "avatar.jpg",
        base64Data,
        "base64",
        (err) => {
          if (err) throw err;
...
const openchatpage = async function (req, res, next) {
  ...
  if (req.query.id && !user.friends.includes(req.query.id)) {
    ...
    mainallfriendlastmessage = mainallfriendlastmessage.concat(
      allfriendlastmessage,
      allfriendlastmessage2
    );
    ...
    if (req.query.id) {
      ...
      if (!message) {
        message = await Chat.findOne({
          kullanici2: req.user.id,
          kullanici1: req.query.id,
        });
      }
    } else {
      console.log("burada");
    ...
    let okundubilgisi = await Okundu.find({
      user1: { $in: [req.query.id, req.user.id] },
      user2: { $in: [req.query.id, req.user.id] },
    });
    ...
    res.render("chat.ejs", {
      ...
      allfriendlastmessage: mainallfriendlastmessage,
      okundu: okundubilgisi,
      kontrolid: req.query.id,
    });
  }
};
